Shader "Toon"
{
    Properties
    {
        ...
        _ScrollOpacity      ("Scroll Opacity", Range(0, 1))        = 1.0
    }
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D(_ScrollTex); SAMPLER(sampler_ScrollTex);
            ...
            float4 frag(Varyings input) : SV_Target
            {
               ...
               float shadowNL = smoothstep(_ShadowStep - _ShadowStepSmooth, _ShadowStep + _ShadowStepSmooth, NL);
                ...
                // Scrolling texture
                float2 scrollUV = uv + (_Time.y * _ScrollSpeed.xy);
                float4 scrollTex = SAMPLE_TEXTURE2D(_ScrollTex, sampler_ScrollTex, scrollUV);
                
                // Apply opacity to scrolling texture
                float3 finalColor = lerp(diffuse + ambient + specular, scrollTex.rgb, scrollTex.a * _ScrollOpacity);
